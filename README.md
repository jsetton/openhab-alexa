...
[![test](https://github.com/openhab/openhab-alexa/actions/workflows/test.yml/badge.svg)](https://github.com/openhab/openhab-alexa/actions/workflows/test.yml)
...
You will have to install the latest [ASK CLI](https://developer.amazon.com/docs/smapi/quick-start-alexa-skills-kit-command-line-interface.html), and then initialize it:
```
...
To change your deploy region, update the `awsRegion` parameter in the skill's ASK CLI resources file:
...
3. Setup skill account linking using the skill id displayed in previous step and your OAuth2 provider configuration:
    ...
    2. Update skill account linking information:
        ```
...
2. If updating from ASK CLI v1:
    1. Copy relevant settings from `lambda/smarthome/config.js` (ask-cli-v1) to `lambda/config.js` (ask-cli-v2). If configuring `baseURL`, be aware that it should now point to your server root level and not the rest endpoint. Additionally, move your client certificate to `lambda/ssl` if using that authentication method.

    2. Create ask directory and states file adding the skill id listed under your [Alexa developer console](https://developer.amazon.com/alexa/console/ask). This will prevent duplicate skills from being created under your account.
        ```
