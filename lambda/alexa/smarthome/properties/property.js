...
class AlexaProperty {
  ...
  constructor(name, component, tag, parameters, item) {
    ...
    this.item = item && {
      ...
      // Use item channel metadata if available, to determine item binding & thing type
      binding: item.metadata && item.metadata.channel ? item.metadata.channel.value.split(':')[0] : item.binding,
      thingType: item.metadata && item.metadata.channel ? item.metadata.channel.value.split(':')[1] : item.thingType
    };
  }
  ...
  /**
   * Returns if property is reportable
   * @return {Boolean}
   */
  get isReportable() {
    return true;
  ...
  /**
   * Returns if property is proactively reported
   * @return {Boolean}
   */
  ...
  /**
   * Returns if property is retrievable
   * @return {Boolean}
   */
  get isRetrievable() {
  ...
  hasRequiredLinkedProperties(properties) {
    return this.requiredLinkedProperties.every((name) =>
      properties.find((property) => property.name === name && property.tag === this.tag)
    );
  }

  /**
   * Returns if property is valid based on following criteria:
   *    1) has required linked properties
   *
   * @param  {Array}  properties
   * @return {Boolean}
   */
  isValid(properties) {
    return this.hasRequiredLinkedProperties(properties);
  }
