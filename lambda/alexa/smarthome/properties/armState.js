...
class ArmState extends AlexaProperty {
  ...
  isValid(properties) {
    return (
      ...
      // Valid if at least two supported arm states with disarmed state defined
      this.supportedArmStates.length >= 2 &&
      this.supportedArmStates.includes(ArmState.DISARMED)
    );
  }
