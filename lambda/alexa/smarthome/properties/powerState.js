...
class PowerState extends AlexaProperty {
  ...
  isValid(properties) {
    return (
      ...
      // Valid if value map not required, or if every supported values defined in value map
      (!this.requiresValueMap || this.supportedValues.every((value) => typeof this.valueMap[value] !== 'undefined'))
    );
  }
}
