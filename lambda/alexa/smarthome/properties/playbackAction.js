...
class PlaybackAction extends AlexaProperty {
  ...
  get defaultActionMap() {
    switch (this.item.type) {
      case ItemType.ROLLERSHUTTER:
        return { [PlaybackAction.STOP]: ItemValue.STOP };
      case ItemType.SWITCH:
        return { [PlaybackAction.PAUSE]: ItemValue.OFF, [PlaybackAction.RESUME]: ItemValue.ON };
      default:
        return {};
    }
  }
   ...
   */
  get actionMappings() {
    return this.parameters[Parameter.ACTION_MAPPINGS] || this.defaultActionMap;
  }
  ...
  isValid(properties) {
    return (
      ...
      // Valid if has supported operations
      this.supportedOperations.length > 0
    );
  }
