...
class Generic extends DecoupleState {
  ...
  isValid(properties) {
    return (
      ...
      // Valid if friendly names not empty
      this.friendlyNames.length > 0
    );
  }
  ...
  updateParameters(item, metadata, settings) {
    ...
    parameters[Parameter.CAPABILITY_NAMES] = capabilityNames.filter(
      (label = '', index, array) => AlexaCapabilityResources.isValidLabel(label) && array.indexOf(label) === index
    );
