...
class Mode extends Generic {
  ...
  /**
   * Returns if property is non-controlable
   * @return {Boolean}
   */
  ...
  isValid(properties) {
    return (
      ...
      // Valid if at least two supported modes defined
      Object.keys(this.supportedModes).length >= 2
    );
  }
  ...
  updateParameters(item, metadata, settings) {
    ...
    //  1) using supported commands parameter values if defined and property supports commands only,
    //    removing invalid values and seting default command labels if not defined
    //  2) using supported modes parameter values if defined
    //  3) using item state description options if available
    //  4) empty object
    const modes =
      parameters[Parameter.SUPPORTED_COMMANDS] && this.supportsCommandsOnly
        ...
        ? parameters[Parameter.SUPPORTED_MODES]
        : item.stateDescription && item.stateDescription.options.length > 0
