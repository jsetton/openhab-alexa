...
class Input extends AlexaProperty {
  ...
  /**
   * Returns normalized input name
   * @param  {String} value
   * @return {String}
   */
  getInputName(value) {
    // Normalize input name if defined, adding space in front of number at the end (e.g. 'HDMI1' => 'HDMI 1')
    return value && value.replace(/(\S)(\d+)$/, '$1 $2').toUpperCase();
  }
   ...
   */
  getState(value) {
    value = this.getInputName(value);

    // Return state if supported value
    if (this.supportedValues.includes(value)) {
      return value;
    }
  }
  ...
  isValid(properties) {
    return (
      ...
      // Valid if supported inputs not empty
      this.supportedInputs.length > 0
    );
  }
  ...
  updateParameters(item, metadata, settings) {
    ...
    parameters[Parameter.SUPPORTED_INPUTS] = inputs
      .map((value) => this.getInputName(value))
      .filter((value) => this.supportedValues.includes(value));
  }
}
