...
class AlexaMetadata {
  ...
  /**
   * Returns if has config parameter
   * @param  {String}  parameter
   * @return {Boolean}
   */
  hasConfigParameter(parameter) {
  ...
  static convertValue(value, type) {
    ...
    try {
      switch (conversion) {
        ...
        case `string->${ParameterType.RANGE}`: // 'minRange:maxRange' => [minRange, maxRange]
          return (current === 'string' ? value.split(':') : Object.values(value)).map((value) => parseFloat(value));
