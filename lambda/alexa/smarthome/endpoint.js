...
class AlexaEndpoint {
   ...
   */
  get capabilities() {
    return this._capabilities.filter((capability) => capability.isValid).slice(0, AlexaEndpoint.#CAPABILITIES_LIMIT);
  }
  ...
  get discoverableCapabilities() {
    return this.capabilities.filter((capability) => capability.isDiscoverable);
  }
  ...
  addItemCapabilities(item, settings) {
    ...
    for (const value of metadata.values) {
      ...
      if (device) {
        ...
        for (const capability of device.getCapabilities(item, metadata)) {
    ...
    // Add display categories if not group endpoint, using stored list fallback to metadata config category parameter
    if (!this.isGroup) {
      this.addDisplayCategories(categories.length > 0 ? categories : [].concat(metadata.config.category || []));
    }
  }

  /**
   * Dumps capabilities array to string format
   * @return {String}
   */
  dumpCapabilities() {
  ...
  setDetails(item, settings) {
    ...
    this.friendlyName = metadata.config.name || item.label;
  ...
  toJSON() {
    return {
      ...
      cookie: {
        capabilities: this.dumpCapabilities()
      },
  ...
  static fromItem(item, settings) {
    ...
    // Return if endpoint friendly name not defined
