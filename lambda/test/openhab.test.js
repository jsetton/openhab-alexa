...
describe('OpenHAB Tests', () => {
  ...
  describe('get all items', () => {
    // set default environment
    const items = [
      { name: 'foo', state: '42', type: 'Dimmer' },
      { name: 'bar', state: 'OK', type: 'Switch' },
      { name: 'baz', state: 'UNDEF', type: 'String' }
    ];
    const qs = {
  ...
  describe('get server settings', () => {
    it('oh2.4', async () => {
      ...
      nock(baseURL)
        ...
        .reply(200, { language: 'en', measurementSystem: 'US', region: 'US' })
        // uuid
        .get('/rest/uuid')
        .reply(200, '00000000-0000-0000-0000-000000000000');
      // run test
      expect(await openhab.getServerSettings()).to.deep.equal({
        regional: { language: 'en', measurementSystem: 'US', region: 'US' },
        runtime: { uuid: '00000000-0000-0000-0000-000000000000', version: '2' }
      });
      expect(nock.isDone()).to.be.true;
    });
    ...
    it('oh3.x', async () => {
      ...
      nock(baseURL)
        // root resource
        .get('/rest')
        .reply(200, { version: '4', locale: 'en_US', measurementSystem: 'US', runtimeInfo: { version: '3.0.0' } })
        // uuid
        .get('/rest/uuid')
        .reply(200, '00000000-0000-0000-0000-000000000000');
      // run test
      expect(await openhab.getServerSettings()).to.deep.equal({
        regional: { language: 'en', measurementSystem: 'US', region: 'US' },
        runtime: { uuid: '00000000-0000-0000-0000-000000000000', version: '3.0.0' }
      });
      expect(nock.isDone()).to.be.true;
    });
    ...
    it('oh3.x with unauthorized uuid', async () => {
      ...
      expect(await openhab.getServerSettings()).to.deep.equal({
        regional: { language: 'en', measurementSystem: 'US', region: 'US' },
        runtime: { uuid: undefined, version: '3.0.0' }
      });
      expect(nock.isDone()).to.be.true;
    });
